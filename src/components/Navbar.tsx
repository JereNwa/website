  ...
  return (
    <nav className="w-full py-4 px-8 flex items-center justify-between bg-[#ffffff] dark:bg-background text-foreground dark:text-foreground outline-1 outline-[#808080] sticky top-0 z-20">
      <div className="text-xl font-bold">
        <Link
          href="/"
          className="font-sans hover:no-underline flex dark:text-foreground"
        >
          JereNwa
        </Link>
      </div>
      ...
      {isOpen && (
        <motion.ul
          ...
          className="absolute top-16 left-0 w-full h-screen bg-[#ffffff]/90 dark:bg-background/90 shadow-md flex flex-col items-left lg:hidden z-10 space-y-4 py-4 px-8 font-serif rounded-b-md"
        >
          {menus.map((item, idx) => (
            <li key={idx} className="list-none">
              <Link
